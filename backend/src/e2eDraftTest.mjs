...
async function main() {
  try {
    ...
    // get products
  const pRes = await fetch(`${API}/products`, { headers: buildHeaders(token) });
    const products = await pRes.json();
    console.log('Products before:', products.map(p => ({ id: p.id, stock: p.stock })));

    // create draft order with first product qty 1
    const first = products[0];
  const createRes = await fetch(`${API}/orders`, { method: 'POST', headers: buildHeaders(token, { 'Content-Type':'application/json' }), body: JSON.stringify({ items: [{ productId: first.id, quantity: 1 }], status: 'draft' }) });
    const createBody = await createRes.json();
    console.log('Create draft status', createRes.status, createBody);

    // check product stock after draft
  const pRes2 = await fetch(`${API}/products`, { headers: buildHeaders(token) });
  ...
  const sendRes = await fetch(`${API}/orders/${orderId}/status`, { method: 'PUT', headers: buildHeaders(token, { 'Content-Type':'application/json' }), body: JSON.stringify({ status: 'pending' }) });
    const sendBody = await sendRes.json();
    console.log('Send to pending status', sendRes.status, sendBody);

    // check product stock after sending
  const pRes3 = await fetch(`${API}/products`, { headers: buildHeaders(token) });
