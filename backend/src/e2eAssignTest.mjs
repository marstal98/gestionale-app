...
async function run(){
  ...
  const pRes = await fetch(`${BASE}/api/products`, { headers: { Authorization: `Bearer ${token}` } });
  const products = await pRes.json();
  console.log('products', products.map(p=>({id:p.id, stock:p.stock})).slice(0,3));

  const uRes = await fetch(`${BASE}/api/users`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  res = await fetch(`${BASE}/api/orders`, {method:'POST', headers:{'Content-Type':'application/json', Authorization:`Bearer ${token}`}, body: JSON.stringify({ items: [{ productId: product.id, quantity: 1 }], assignedToId: assignee.id })});
  ...
  res = await fetch(`${BASE}/api/orders/${created.id}/assign`, {method:'PUT', headers:{'Content-Type':'application/json', Authorization:`Bearer ${token}`}, body: JSON.stringify({ assignedToId: newAssignee.id })});
